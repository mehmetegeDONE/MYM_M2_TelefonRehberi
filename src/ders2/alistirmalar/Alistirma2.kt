...
fun main() {
    ...
    Ancak KDV oranına hiç dokunmayıp varsayılan değerinde (0.18) kalmasını sağlamalısınız.
    Bunu aradaki parametreyi atlayacak şekilde isimlendirilmiş argüman (Named Argument) kullanarak gerçekleştirin.
     */

    print("Satın aldığınız ürünün fiyatını ondalıklı olarak girin: ")
    val fiyat = readln().toDoubleOrNull()?: 0.0

    print("Ucretsiz kargo kuponunuz var mı (E/H): ")
    val kuponVarMi = readln().trim().equals("e",true)

    val toplamTutar = if (kuponVarMi) {
        sepetHesapla(fiyat, kargoUcreti = 0.0)
    } else {
        sepetHesapla(fiyat)
    }
